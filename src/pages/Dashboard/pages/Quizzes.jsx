...
import UpsertQuizz from "../components/modal/UpsertQuizz";
import TableLoader from "../../../components/loader/TableLoader";
import ErrorMessage from "../../../components/ErrorMessage";
import { useFetchQuizesQuery } from "../../../redux/featuers/quizes/quizApi";
const Quizzes = () => {
  ...
  const { data: quizes, isLoading, isError } = useFetchQuizesQuery();
  ...
  const handleDelete = (id) => {
    ...
    setConfirmation({
      data: id,
      popupType: "deleteQuiz",
      popupMessage: "Are you sure you want to delete this Quiz?",
    });
  };
  ...
  if (!isLoading && !isError && quizes?.length === 0) {
    content = <ErrorMessage message={"No quizes found"} />;
  }

  if (!isLoading && !isError && quizes?.length > 0) {
    content = <Table headers={headers} rowData={quizes} />;
  }
